...
def plot_decision_boundary(pred_func, X, y):
    ...
    x_min, x_max = X0[:, 0].min() - .5, X0[:, 0].max() + .5
    y_min, y_max = X0[:, 1].min() - .5, X0[:, 1].max() + .5
    h = 0.01
    # Generate a grid of points with distance h between them
    xx, yy = np.meshgrid(np.arange(x_min, x_max, h), np.arange(y_min, y_max, h))
    #xx, yy = np.meshgrid(np.arange(x_min.eval(), x_max.eval(), h), np.arange(y_min.eval(), y_max.eval(), h))

    # Predict the function value for the whole gid
    #Z = pred_func(np.c_[xx.ravel(), yy.ravel()])
    X.set_value(np.c_[xx.ravel().astype('float32'), yy.ravel().astype('float32')])
    Z = pred_func()
    Z = Z.reshape(xx.shape)
    # Plot the contour and training examples
    plt.contourf(xx, yy, Z, cmap=plt.cm.Spectral)
    plt.scatter(X0[:, 0], X0[:, 1], c=y, cmap=plt.cm.Spectral)
    plt.show()
...
plt.title("Decision Boundary for hidden layer size %i" % (nn_hdim))
plot_decision_boundary(lambda : predict(), X, train_y)
